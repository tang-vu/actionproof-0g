...
| 2026-08-15 | Compute live status   | Galileo live proof uses current catalog model `qwen2.5-omni`; retain request/provider metadata and recheck `/v1/models` before future runs.                        | Two paid structured assessments passed strict runtime validation. No malformed-output repair or sandbox fallback was used.                                                                                                                     |
| 2026-08-15 | Explorer provenance   | Query ChainScan `/open/api?module=contract&action=getsourcecode` during simulation and validate its response; degrade to `unknown` on timeout/malformed response.  | The deployed demo sources are verified. Earlier live attempts correctly remained `review` while the adapter reported provenance as unknown; truthful source evidence enabled the safe execution without weakening policy.                      |
...
| 2026-08-17 | ERC-8004 registration | Add a separate Galileo-only, dry-run-first registration tool; keep it outside analysis orchestration.                                                              | The current official spec defines `register()`, `setAgentURI()`, the registration-file shape, and initial `agentWallet=owner`. The tool enforces the exact action-agent wallet and a distinct broadcast confirmation.                          |
